...
Use `/home/azrael/.local/bin/minion-hq-deploy` to check, commit, push the task branch, and install. Verify that its symlink targets this task's `deploy.sh`; update the symlink when continuing in a different Minion HQ worktree. This absolute command is the scoped Codex allow rule. Direct `./deploy.sh` remains available but may require approval. Preserve unrelated user changes.
